...
contract("PreIco test", function(accounts) {
	...
	it("should have correct buy amount if buys more than supply", function() {
            ...
            assert.isBelow(balanceAfter.valueOf() - 7000*10**18-500, 1, "Balance after refund is wrong!")
        });
    	});
	...
	it("time test", function() {
		...
		return PreIco.deployed().then(function (instance) {
			ico = instance;
			return ico.getTime.call(accounts[0]);
	}).then(function(supply) {
		assert.equal(supply.valueOf(), 500, "Supply isn't 500!");
	});
	});
